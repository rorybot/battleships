...
describe 'Board analysis' do
  ...
  it 'can inspect the a number of rows along' do
    ...
    expect(board.radar([:A1, :A2, :A3])).to eq("Bzzt - no can do! A1 is free!, A2 is blocked!, A3 is free!")
  end

  it 'can inspect the row above at same column' do
    board = Board.new
    board.row_c[:c1] = 'battleship'
    expect(board.radar([:A1, :B1, :C1])).to eq("Bzzt - no can do! A1 is free!, B1 is blocked!, C1 is blocked!")
  end
